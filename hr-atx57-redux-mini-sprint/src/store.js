...
const usersReducer = function(previousState = { userList: [] }, action) {
  ...
  switch (action.type) {
    case "ADD_USER":
      ...
      /* fill me in */
    case "ADD_MESSAGE":
      if (!previousState.userList.includes(action.payload.userName)) {
        return [...previousState.userList, action.payload];
      } else {
        return [...previousState.userList];
      }
    default:
      return previousState;
  }
...
store.dispatch(
 ...
 store.dispatch(
  ...
  store.dispatch(addNewMessageActionCreator({userName: 'BonnyAnne', text: "IMMA SMASH THEM EGGS"}));
  store.dispatch(addUserActionCreator({userName: "Billy_Blue_Bonnet"}));
